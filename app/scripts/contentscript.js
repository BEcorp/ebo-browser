...
async function setupStreams () {
  ...
  pump(
    ...
    pageStream,
    pageMux,
    (err) => logStreamDisconnectWarning('EBO Inpage Multiplex', err)
  )
  pump(
    extensionMux,
    extensionStream,
    extensionMux,
    (err) => logStreamDisconnectWarning('EBO Background Multiplex', err)
  )
...
function forwardTrafficBetweenMuxers (channelName, muxA, muxB) {
  ...
  pump(
    ...
    channelB,
    channelA,
    (err) => logStreamDisconnectWarning(`EBO muxed traffic for channel "${channelName}" failed.`, err)
  )
}
...
function logStreamDisconnectWarning (remoteLabel, err) {
  let warningMsg = `EBOContentscript - lost connection to ${remoteLabel}`
  if (err) warningMsg += '\n' + err.stack
